...
const FollowerRow = ({ follower, authUserId }) => {
  return (
    <li
    ...
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        <img
          src={normalizeAvatar(follower.avatar)}
      ...
      {follower._id !== authUserId && <FollowButton userId={follower._id} />}
    </li>
  );
};
...
export default function FollowersPage() {
  ...
  const filteredList = useMemo(() => {
    ...
    return list.filter((follower) => {
      const hay = `${follower.name || ""} ${follower.username || ""}`
        .toLowerCase();
        ...
        <div className="relative px-5 py-5 sm:px-6">
          ...
          <div className="relative flex flex-wrap items-start justify-between gap-4">
            ...
            <div className="text-left sm:text-right">
              ...
              <p className="text-xs text-slate-500">People who follow this account.</p>
          ...
          <div className="relative mt-4 flex flex-wrap items-center gap-3">
            ...
            <div className="text-xs text-slate-500 font-medium">
              {filteredList.length} of {list.length} followers
            </div>
          </div>
        </div>
